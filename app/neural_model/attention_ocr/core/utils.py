...
from app.neural_model.attention_ocr.vocabulary import Vocabulary
from app.neural_model.attention_ocr.config_model import *
from app.neural_model.attention_ocr.image_util import ImageUtil
from math import *
...
# def draw_bbox_old(image, bboxes):
...
def draw_bbox(original_image, bboxes):
    fontScale = 0.5
    ...
    for box in bboxes:
        ...
        t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
        c3 = (c1[0] + t_size[0], c1[1] - t_size[1] - 3)    
        img_bbox = cv2.rectangle(original_image, c1, c2, bbox_color, bbox_thick)
        cv2.rectangle(img_bbox, c1, (np.float32(c3[0]), np.float32(c3[1])), bbox_color, -1) #filled
        cv2.putText(img_bbox, bbox_mess, (c1[0], np.float32(c1[1] - 2)), cv2.FONT_HERSHEY_SIMPLEX,
                fontScale, (0, 0, 0), bbox_thick // 2, lineType=cv2.LINE_AA)    

    return img_bbox

# function for cropping each detection and saving as new image
def crop_objects(img, boxes, i, classes):
    ...
    if class_name in allowed_classes:
        ...
        # crop detection from image (take an additional 1 pixels around all edges)
    ...
    return img_show, xmin, ymin, xmax, ymax

def predict_crop(model, img_crop):
    result = ''
    v = Vocabulary()
    height, width = model.h, model.w    
    image_util = ImageUtil(image_height=image_height, image_width=image_width)
    img_tensor = image_util.load(img_crop)
    ...
    results_process = re.sub(r"[^a-zA-Z0-9.\-\s]", "", result) 
